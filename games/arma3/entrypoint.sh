...
EGG_URL='https://github.com/blademindeu/eggs/tree/master/game_eggs/steamcmd_servers/arma/arma3'   # URL for Pterodactyl Egg & Info (only used as info to legacy users)
...
function RunSteamCMD { #[Input: int server=0 mod=1 optional_mod=2; int id]
    ...
    while (( $updateAttempt < $STEAMCMD_ATTEMPTS )); do # Loop for specified number of attempts
        ...
        if [[ $1 == 0 ]]; then # Server
            numactl --physcpubind=+0 ${STEAMCMD_DIR}/steamcmd.sh +force_install_dir /home/blademind "+login \"${STEAM_USER}\" \"${STEAM_PASS}\"" +app_update $2 $extraFlags $validateServer +quit | tee -a "${STEAMCMD_LOG}"
        else # Mod
        ...
        if [[ -n ${loggedErrors} ]]; then # Catch errors (ignore setlocale, SDL, steamservice, and thread priority warnings)
            ...
            cp -r /tmp/dumps /home/blademind/dumps
...
LOG_FILE="/home/blademind/serverprofile/rpt/arma3server_$(date '+%m_%d_%Y_%H%M%S').rpt"

# Ensure the logs directory exists
mkdir -p /home/blademind/serverprofile/rpt
